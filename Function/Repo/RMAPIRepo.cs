...
namespace Walmart
{
    ...
    public class RMAPIRepo
    {
        ...
        public async Task<bool> AssigmentDenyPolicy(string GroupName, string SubscriptionID, string policyNoDeployID)
        {   
        ...
        public async Task<bool> AssigmentLimitedPolicy(string GroupName, int MaxSizevCore , List<string> AvailableSizes, string SubscriptionID, string policyLimitedID)
        {
            var policyAssiment = new PolicyAssigment();
            policyAssiment.properties = new Properties()
            {
        ...
        public async Task<bool> IsDenyPolicy(string GroupName, string SubscriptionID, string policyNoDeployID)
        {
            var queryToken = await GetS2SAccessToken(_TenantId, _AppID, _AppKey);
            ...
            return await retry.ExecuteAsync(async () =>
            {
                using (var client = new HttpClient())
                {
                    ...
                    _log.LogInformation($"DenyPolicy found for gr:{GroupName}");
                    return result.IsSuccessStatusCode;
                }
            });
        ...
        public async Task<bool> IsLimitedPolicy(string GroupName, string SubscriptionID, string policyLimitedID)
        {
            var queryToken = await GetS2SAccessToken(_TenantId, _AppID, _AppKey);
            ...
            return await retry.ExecuteAsync(async () =>
            {
                using (var client = new HttpClient())
                {
                    ...
                    var url = new Uri($"https://management.azure.com/subscriptions/{SubscriptionID}/resourceGroups/{GroupName}/providers/Microsoft.Authorization/policyAssignments/{policyLimitedID}?api-version=2017-06-01-preview");
                    var result = await client.GetAsync(url);
                    _log.LogInformation($"LimitedPolicy found for gr:{GroupName}");
                    return result.IsSuccessStatusCode;
                }
            });
        }

        public async Task<bool> RemoveDenyPolicy(string GroupName, string SubscriptionID, string policyNoDeployID)
        {
            var queryToken = await GetS2SAccessToken(_TenantId, _AppID, _AppKey);
        ...
        public async Task<bool> RemoveLimitedPolicy(string GroupName, string SubscriptionID, string policyLimitedID)
        {
