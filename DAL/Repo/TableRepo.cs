...
#region Settings
        ...
        public static void UpdateSetting(SettingTableItem sett)
        {
            TableOperation insertOperation = TableOperation.Merge(sett);
            settingsTable.ExecuteAsync(insertOperation);
        }
        ...
        public static async Task<Tuple<int, int>> GetGroupQuote(string GroupName, string SubID)
        {
            ...
            TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, GroupTableItem.GetRowkey(GroupName, SubID)), TableOperators.And,
                TableQuery.GenerateFilterConditionForBool("Enabled", QueryComparisons.NotEqual, false)));
            ...
            if (group == null)
            {
                ...
                group = new GroupTableItem(GroupName, SubID) { QuoteLimit = -1, Enabled = true, ReviewDate = DateTime.Now, GroupName =GroupName };
                CreateNewGroup(group);
            }
            return new Tuple<int, int> (group.QuoteLimit, group.CurrentVCore);

        }

        public static void CreateNewGroup(GroupTableItem newgroup)
        {
            ...
            groupTable.ExecuteAsync(insertOperation);
        }
        ...
        public static async Task<bool> DeleteResourceGroupInfo(string SubID,string affectedGroupName)
        {
            var group = await GetGroup(affectedGroupName, SubID);
            if (group == null) return false; //already deleted or not found
            TableOperation del = TableOperation.Delete(group); 
            var result = await groupTable.ExecuteAsync(del);
            return result.HttpStatusCode == (int)HttpStatusCode.NoContent;
        }

        public static void UpdateGroup(GroupTableItem group)
        {
